...
public partial class ModVersionsWindowViewModel : ObservableRecipient
{
    ...
    public List<AbstractModVersion> RenderedModVersions
    {
        get
        {
            var newList = AllModVersions.Where(x => x.GameVersions.Contains(SelectedGameVersion)).ToList();
    ...
    [ObservableProperty] private string providerKey = "modrinth";
    ...
    [ObservableProperty] private bool loadingModVersion = false;

    [ObservableProperty] private bool showChangeLog = Settings.Default.ShowChangeLog;

    [ObservableProperty]
    [NotifyPropertyChangedFor(nameof(RenderedModVersions))]
    private string? selectedGameVersion;

    [ObservableProperty] private string? selectedModVersionId;
    #endregion
